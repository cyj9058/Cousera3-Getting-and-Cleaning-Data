...
#page 18,19  [A-z0-9_] 는 숫자+문자를 검색한다. \w는 더이상 통용 되지 않는다. [^ ]을 넣으면  부정을 의미한다.
select ' "나는 에너지 에너지 에너지 산업에 종사하고 싶다." ' regexp '[ㄱ-ㅎ가-힣0-9_]';
select '에너지' regexp '[^ㄱ-ㅎ가-힇]'; 
select '에너지' regexp '\\w'; 

#page20
select '에너지 에너지' regexp '[[:space:]]'; #공백을 찾는다. 
